<div>
    ...
    <x-jet-dialog-modal wire:model="show">
        ...
        {{--END the Content--}}
        <x-slot name="footer">
            <x-buttons.turquoise wire:loading.remove wire:target="template" wire:click="save_stoks">
                {{__('Guardar')}}
            </x-buttons.turquoise>
            <x-buttons.default @Click="show = !show">
                {{__('Cerrar')}}
            </x-buttons.default>
            <x-banners.progress wire:loading wire:target="save_stoks"/>
        </x-slot>
    </x-jet-dialog-modal>

<div>

    <x-containers.secondary>
        <div class="flex relative text-center py-2 ">
            <h1 class="text-3xl tracking-wider text-white text-sha uppercase font-black  p-4 self-center z-10 content-center absolute text-center w-full md:text-4xl"></h1>
            <img class="w-full object-cover h-72 block mx-auto  sm:block sm:w-full" 
            src="https://tallerdeltotebag.es/wp-content/uploads/2019/10/que-son-las-ofertas-flash.jpg"
            alt="Banner" width="1920" height="288" />
        </div>
    </x-containers.secondary>

    </br>

    <x-containers.secondary>
        <div class="space-y-10 my-6 md:space-y-0 md:grid md:grid-cols-4 md:gap-x-8 md:gap-y-10 ">
                <span class="bg-clip-text text-transparent bg-gradient-to-r from-blue-700 to-yellow-500 font-bold text-center text-5xl font-extrabold ">
                    Ofertas
                    
                </span>
            
                <a href="" class="cursor-not-allowed grid grid-cols-1 text-blue-500 font-bold  hover:text-yellow-500 ">
                    !Encuentra precios increíbles cada día!
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 " fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
                    </svg>
                </a>
            
            
            <section class="space-y-0 md:space-y-0 md:grid md:grid-cols-3 md:gap-x-3 md:gap-y-10 ">
                
                <div class="p-2 md:w-24 ">
                    <div class="flex justify-around items-center px-2 py-3 bg-gray-200 rounded-lg shadow-xs cursor-wait bg-blue-900 hover:bg-yellow-600 hover:text-black text-white">
                        <div>
                        <p class="text-xs font-medium ml-2 ">
                          Ofertas Relampago
                        </p>

                        </div>
                    </div>
                </div>
                <div class="p-2 md:w-24">
                    <div class="flex justify-around items-center px-2 py-3 bg-gray-200 rounded-lg shadow-xs cursor-wait bg-blue-900 hover:bg-yellow-600 hover:text-black text-white">
                        <div>
                        <p class="text-xs font-medium ml-2 ">
                          Menos de 500
                        </p>

                        </div>
                    </div>
                </div>
               
            </section>
        </div>
    </x-containers.secondary>
        
    <br>
    
    

    <x-containers.secondary>
        <div class="px-3 md:px-0 py-6 grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-10">
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    Sax Shields
                </x-slot>
                <x-slot name="name">
                    Kit de Snaker Shields 2 Pares (4 piezas) Escoge tus colores
                </x-slot>
                <x-slot name="price">
                    $98.00
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    SAX Comercializadora
                </x-slot>
                <x-slot name="name">
                    Sneakers Rain Convers Cubre Tenis Lluvia Protect
                </x-slot>
                <x-slot name="price">
                    $135.15
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    Bogi
                </x-slot>
                <x-slot name="name">
                    Pantalon De Mezclilla Premium Uso Rudo Talla L XL 38 40 42
                </x-slot>
                <x-slot name="price">
                    $249.00
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    SAX Comercializadora
                </x-slot>
                <x-slot name="name">
                    Sneakers Rain Convers Cubre Tenis Lluvia Protect
                </x-slot>
                <x-slot name="price">
                    $135.15
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    Sax Shields
                </x-slot>
                <x-slot name="name">
                    Kit de Snaker Shields 2 Pares (4 piezas) Escoge tus colores
                </x-slot>
                <x-slot name="price">
                    $98.00
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    SAX Comercializadora
                </x-slot>
                <x-slot name="name">
                    Sneakers Rain Convers Cubre Tenis Lluvia Protect
                </x-slot>
                <x-slot name="price">
                    $135.15
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    Bogi
                </x-slot>
                <x-slot name="name">
                    Pantalon De Mezclilla Premium Uso Rudo Talla L XL 38 40 42
                </x-slot>
                <x-slot name="price">
                    $249.00
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    SAX Comercializadora
                </x-slot>
                <x-slot name="name">
                    Sneakers Rain Convers Cubre Tenis Lluvia Protect
                </x-slot>
                <x-slot name="price">
                    $135.15
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    Sax Shields
                </x-slot>
                <x-slot name="name">
                    Kit de Snaker Shields 2 Pares (4 piezas) Escoge tus colores
                </x-slot>
                <x-slot name="price">
                    $98.00
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    SAX Comercializadora
                </x-slot>
                <x-slot name="name">
                    Sneakers Rain Convers Cubre Tenis Lluvia Protect
                </x-slot>
                <x-slot name="price">
                    $135.15
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    Bogi
                </x-slot>
                <x-slot name="name">
                    Pantalon De Mezclilla Premium Uso Rudo Talla L XL 38 40 42
                </x-slot>
                <x-slot name="price">
                    $249.00
                </x-slot>
            </x-cards.secondary>
            <x-cards.secondary :link="'#'">
                <x-slot name="image">
                    imagen
                </x-slot>
                <x-slot name="trademark">
                    SAX Comercializadora
                </x-slot>
                <x-slot name="name">
                    Sneakers Rain Convers Cubre Tenis Lluvia Protect
                </x-slot>
                <x-slot name="price">
                    $135.15
                </x-slot>
            </x-cards.secondary>
        </div>
    </x-containers.secondary>
    



</div>
